			...
			$('#confirmshare').click(function() {
			  ...
			  var content = snapshot.val().content;
				...
				var dockey = snapshot.key;
					...
					var updates = {};
					updates['docs/users/' + uid + '/docs/' + doctoload + '/shared'] = 'index.html?shareddoc=' + dockey;
					
					firebase.database().ref().update(updates);
				$('#sharelink').html('<a>rosho123.github.io/index.html?shareddoc=' + dockey + '</a>');
					var sharedoclink = '<a onclick="location.realod();">index.html?shareddoc=' + dockey + '</a>';
					console.log(sharedoclink);
					$('#sharelink').click(function() {
						 location.reload();
					});
					$('#loader-share').hide();
				console.log('index.html?shareddoc=' + dockey);
				});
		...
		profile_picture : photoUrl
		 ...
		 firebase.database().ref('docs/users/' + uid + '/settings/welcomedoc').once('value').then(function(snapshot) {
		  var docisthere = snapshot.val();
			 if (!docisthere) {
				 var encwelcome = blowfish.encrypt( "Welcome to Simple Docs",  uid,  {outputType: 1, cipherMode: 0});
