    ...
    firebase.auth().onAuthStateChanged(function(user) {
        if (user) {
            ...
            if (doctoload || shareddoc) {
                ...
                $('#fontsettingsli').click(function(event) {
                ...
                docloadRef.once('value').then(function(snapshot) {
                    ...
                    $('#confirmshare').click(function() {
                        ...
                        docloadRef.once('value').then(function(snapshot) {
                            ...
                            dcrname = blowfish.decrypt(name, uid, { outputType: 1, cipherMode: 0 });

                            dcryptedcontent = blowfish.decrypt(content, uid, { outputType: 1, cipherMode: 0 });
                    ...
                    setInterval(function() {
                        ...
                        if (enccontnet == content) {
                            ...
                            if (shareddoc) {
                                ...
                                var encname = blowfish.encrypt(currentname, uid, { outputType: 1, cipherMode: 0 });
                                encpreview = blowfish.encrypt(preview, uid, { outputType: 1, cipherMode: 0 });
                            }
                ...
                firebase.database().ref('docs/users/' + uid + '/settings/welcomedoc').once('value').then(function(snapshot) {
                    ...
                    if (!docisthere) {
                        ...
                        preview = blowfish.encrypt(preview, uid, { outputType: 1, cipherMode: 0 });
                        var encwelcome = blowfish.encrypt('<p contenteditible="true" style="margin: 0px; font-size: 40px; line-height: normal; font-family: Monoton; -webkit-text-stroke-width: initial; -webkit-text-stroke-color: rgb(0, 0, 0); color: rgb(66, 66, 66);">Welcome to Simple Docs!</p>', uid, { outputType: 1, cipherMode: 0 });
                ...
                docsRef.on('child_added', function(data) {
                    ...
                    var dycrptcontent = blowfish.decrypt(content, uid, { outputType: 1, cipherMode: 0 })

                    var preview = data.val().preview;

                    if (preview) {
                        preview = blowfish.decrypt(preview, uid, { outputType: 1, cipherMode: 0 })
            ...
            $("#newdocsubmit").click(function(e) {
                ...
                var encnewcontent = blowfish.encrypt(content, uid, { outputType: 1, cipherMode: 0 });
                preview = blowfish.encrypt("New Document", uid, { outputType: 1, cipherMode: 0 });
